...
亮度为 0 的预设默认只应用连接、亮度和对比度，避免息屏已生效却因系统改变横竖屏模式而报错。原分辨率与旋转设置仍保留，可在编辑预设中勾选“应用分辨率与旋转”严格执行，也可单独选择分辨率“保持当前”或旋转“跟随当前”。

### 限制

- 显示器开关使用 macOS 私有接口 `CGSConfigureDisplayEnabled`，不适合发布到 Mac App Store，也可能受系统更新影响
- 修改旋转方向需要 BetterDisplay Pro 及显示器支持；保持现有方向和默认息屏流程不调用旋转接口

...
xcrun clang -fobjc-arc -target "$(uname -m)-apple-macosx13.0" \
  -c "$ROOT/NativeDisplayRotation.m" -o "$OUTPUT/NativeDisplayRotation.o"
for test in "$ROOT"/Tests/*Tests.swift; do
  ...
  xcrun swiftc -D TESTING -target "$(uname -m)-apple-macosx13.0" \
    ...
    "$OUTPUT/NativeDisplayRotation.o" "$ROOT/NativeDisplayRotation.swift" \
